...
import 'dart:io';
import 'package:zipchat/Screens/homepage/Setupdata.dart';
import 'package:zipchat/Services/Providers/SmartContactProviderWithLocalStoreData.dart';
import 'package:zipchat/Services/localization/language.dart';
import 'package:zipchat/Utils/color_detector.dart';
import 'package:zipchat/Utils/custom_url_launcher.dart';
import 'package:zipchat/Utils/setStatusBarColor.dart';
import 'package:zipchat/Utils/theme_management.dart';
import 'package:zipchat/widgets/AllinOneCameraGalleryImageVideoPicker/AllinOneCameraGalleryImageVideoPicker.dart';
import 'package:zipchat/widgets/CameraGalleryImagePicker/camera_image_gallery_picker.dart';
import 'package:zipchat/widgets/CameraGalleryImagePicker/multiMediaPicker.dart';
import 'package:zipchat/widgets/DownloadManager/download_all_file_type.dart';
import 'package:zipchat/widgets/DynamicBottomSheet/dynamic_modal_bottomsheet.dart';
import 'package:zipchat/widgets/VideoEditor/video_editor.dart';
...
import 'package:zipchat/Screens/chat_screen/utils/aes_encryption.dart';
import 'package:zipchat/Screens/chat_screen/utils/uploadMediaWithProgress.dart';
import 'package:zipchat/Screens/contact_screens/SelectContactsToForward.dart';
import 'package:zipchat/Screens/security_screens/security.dart';
import 'package:zipchat/Services/Admob/admob.dart';
import 'package:zipchat/Utils/emoji_detect.dart';
import 'package:zipchat/Utils/mime_type.dart';
import 'package:zipchat/main.dart';
import 'package:zipchat/widgets/CountryPicker/CountryCode.dart';
import 'package:zipchat/Configs/Dbkeys.dart';
import 'package:zipchat/Configs/Dbpaths.dart';
import 'package:zipchat/Screens/chat_screen/utils/deleteChatMedia.dart';
import 'package:zipchat/Screens/privacypolicy&TnC/PdfViewFromCachedUrl.dart';
import 'package:zipchat/Services/Providers/Observer.dart';
import 'package:zipchat/widgets/MultiDocumentPicker/multiDocumentPicker.dart';
import 'package:zipchat/widgets/MyElevatedButton/MyElevatedButton.dart';
import 'package:zipchat/widgets/SoundPlayer/SoundPlayerPro.dart';
import 'package:zipchat/Services/Providers/currentchat_peer.dart';
...
import 'package:zipchat/Screens/chat_screen/utils/audioPlayback.dart';
import 'package:zipchat/Screens/chat_screen/utils/message.dart';
import 'package:zipchat/Screens/contact_screens/ContactsSelect.dart';
import 'package:zipchat/Models/DataModel.dart';
import 'package:zipchat/Screens/chat_screen/utils/photo_view.dart';
import 'package:zipchat/Screens/profile_settings/profile_view.dart';
import 'package:zipchat/Services/Providers/seen_provider.dart';
import 'package:zipchat/Services/Providers/seen_state.dart';
...
import 'package:zipchat/Utils/chat_controller.dart';
import 'package:zipchat/Utils/crc.dart';
import 'package:zipchat/Utils/open_settings.dart';
import 'package:zipchat/Utils/save.dart';
import 'package:zipchat/Utils/utils.dart';
import 'package:zipchat/widgets/AudioRecorder/Audiorecord.dart';
import 'package:zipchat/widgets/ImagePicker/image_picker.dart';
import 'package:zipchat/widgets/VideoPicker/VideoPreview.dart';
import 'package:zipchat/Screens/chat_screen/Widget/bubble.dart';
...
import 'package:zipchat/Models/E2EE/e2ee.dart' as e2ee;
...
import 'package:zipchat/Configs/Enum.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  String decryptWithCRC(String input) {
    ...
    // zipchat.toast(getTranslated(this.context, 'msgnotload'));
    return '';
  }
  ...
  readLocal(
    ...
    chatId = zipchat.getChatId(currentUserNo!, peerNo!);
  ...
  Future uploadFile(bool isthumbnail, {int? timestamp}) async {
    ...
    if (isthumbnail == true) {
      ...
      await _mediaInfo.getMediaInfo(thumbnailFile.path).then((mediaInfo) {
        ...
        zipchat.toast(getTranslated(this.context, 'failedsending'));
  ...
  void _changeLanguage(Language language) async {
    ...
    zipchatWrapper.setLocale(this.context, _locale);
    ...
    if (content.trim() != '') {
      ...
      try {
        ...
        if (encrypted is String) {
          ...
          zipchat.toast('Nothing to encrypt');
        }
      } on Exception catch (_) {
        // debugPrint('Exception caught!');
        zipchat.toast("Exception: $_");
      }
    }
  }
          ...
          builder: (BuildContext popable) => ListTile(
              ...
              onTap: () async {
                ...
                await FirebaseFirestore.instance
                    ...
                    .then((chatDoc) async {
                  if (!chatDoc.exists) {
                    zipchat.toast('Please reload this screen !');
              ...
              onTap: () async {
                ...
                if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true
                  ...
                  await deleteMsgMedia(mssgDoc, chatId!)
                      .then((isDeleted) async {
                    if (isDeleted == false || isDeleted == null) {
                      zipchat.toast('Could not delete. Please try again!');
                    } else {
                      await FirebaseFirestore.instance
                          .collection(DbPaths.collectionmessages)
                          .doc(chatId)
      ...
      tiles.add(Builder(
          builder: (BuildContext popable) => ListTile(
              ...
              onTap: () {
                ...
                zipchat.toast(getTranslated(popable, 'copied'));
                Navigator.of(popable).pop();
              })));
    }
    if (((mssgDoc[Dbkeys.from] == currentUserNo &&
      ...
      tiles.add(Builder(
          builder: (BuildContext popable) => ListTile(
              ...
              onTap: () async {
                Navigator.of(popable).pop();
                try {
                  var v = await zipchat.translateString(
    ...
    if (index >= list.length) {
      ...
      if (list[index].containsKey(Dbkeys.groupNAME)) {
        try {
          ...
          zipchat.toast('Failed to send $e');
        }
      } else {
        try {
          ...
          if (encrypted is String) {
            ...
            zipchat.toast('Nothing to send');
          }
        } catch (e) {
          setStateIfMounted(() {
            isgeneratingSomethingLoader = false;
          });
          zipchat.toast('Failed to Forward message. Error:$e');
        }
      }
    }
  }
                          ...
                          Container(
                              child: Row(
                                ...
                                Icon(
                                  ...
                                  size: 12,
                                  color: zipchatGrey.withOpacity(0.5),
                                ),
                                SizedBox(
                                  width: 5,
                                ),
                                Text(getTranslated(this.context, 'forwarded'),
                                    maxLines: 1,
                                    style: TextStyle(
                                        color: zipchatGrey.withOpacity(0.7),
                                        fontStyle: FontStyle.italic,
          ...
          ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (isUser == null || isUser == false) {
                  zipchat.toast(getTranslated(this.context, 'usernotjoined') +
                      ' $Appname');
                }
              },
  ...
  replyAttachedWidget(BuildContext context, var doc) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(
              color: zipchatWhite.withOpacity(0.55),
          ...
          child: Stack(
            children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      color: zipchatGrey.withOpacity(0.1),
                        ...
                        children: [
                          ...
                          doc[Dbkeys.messageType] == MessageType.text.index
                              ? Text(
                                  ...
                                  maxLines: 1,
                                  style: TextStyle(color: zipchatBlack),
                                )
                              : doc[Dbkeys.messageType] == MessageType.doc.index
                                  ? Container(
      ...
      child: isgeneratingSomethingLoader
          ? Container(
              child: Center(
                child: CircularProgressIndicator(
                    valueColor:
                        AlwaysStoppedAnimation<Color>(zipchatPRIMARYcolor)),
              ),
              color: pickTextColorBasedOnBgColorAdvanced(
                      !Thm.isDarktheme(widget.prefs)
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(color: zipchatGrey, fontSize: 14),
                        )
                      ],
                    ),
                  ),
                          ...
                          onPressed: () async {
                            ...
                            File? selectedMedia =
                                await pickVideoFromgallery(context)
                                    .catchError((err) {
                              zipchat
                                  .toast(getTranslated(context, "invalidfile"));
                              return null;
                            });
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(color: zipchatGrey),
                        )
                      ],
                    ),
                  ),
        ...
        actions: <Widget>[
          ...
          ElevatedButton(
              ...
              child: Text(getTranslated(this.context, 'acpt'),
                  style: TextStyle(color: zipchatPRIMARYcolor)),
              onPressed: () {
                      ...
                      children: [
                        ...
                        Flexible(
                          child: TextField(
                            ...
                            style:
                                TextStyle(fontSize: 16.0, color: zipchatBlack),
                      ...
                      icon: textInSendButton == ""
                          ? new Icon(
                              textEditingController.text.length == 0
                                  ? Icons.mic
                                  : Icons.send,
                              color: zipchatWhite.withOpacity(0.99),
                            )
                          : textEditingController.text.length == 0
                              ? new Icon(
                                  Icons.mic,
                  ...
                  child: SizedBox(
                    ...
                    child: EmojiPicker(
                        ...
                        config: Config(
                            ...
                            indicatorColor: zipchatPRIMARYcolor,
                            iconColor: Colors.grey,
                            iconColorSelected: zipchatPRIMARYcolor,
                            progressIndicatorColor: Colors.blue,
                            backspaceColor: zipchatPRIMARYcolor,
                            showRecentsTab: true,
                            recentsLimit: 28,
        ...
        for (final doc in docs.docs) {
          ...
          try {
            ...
            if (e.toString().contains('range')) {
              zipchat.toast(getTranslated(this.context, 'failedtoloadchat'));
              Navigator.of(this.context).pop();
            }
          }
        }
...
//-- GROUP BY DATE ---
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      prefs: widget.prefs,
      scaffold: zipchat.getNTPWrappedWidget(WillPopScope(
              ...
              child: ScopedModelDescendant<DataModel>(
                ...
                return peer != null
                    ? peer![Dbkeys.accountstatus] == Dbkeys.sTATUSdeleted
                        ? Scaffold(
                            backgroundColor: Thm.isDarktheme(widget.prefs)
                                ? zipchatCHATBACKGROUNDDarkMode
                                : zipchatCHATBACKGROUNDLightMode,
                            appBar: AppBar(
                              backgroundColor: Thm.isDarktheme(widget.prefs)
                                  ? zipchatAPPBARcolorDarkMode
                                  : zipchatAPPBARcolorLightMode,
                                            ...
                                            onSelected: (dynamic val) {
                                              switch (val) {
                                                                    ...
                                                                    myElevatedButton(
                                                                        ...
                                                                        onPressed:
                                                                          ...
                                                                          Map<String, dynamic>
                                                                            ...
                                                                            'id':
                                                                                zipchat.getChatId(currentUserNo!, peerNo!),
                                                                          };
                                                                          ...
                                                                          await FirebaseFirestore
                                                                              ...
                                                                              .then((value) async {
                                                                            showModalBottomSheet(
                                                                                backgroundColor: Thm.isDarktheme(widget.prefs) ? zipchatDIALOGColorDarkMode : zipchatDIALOGColorLightMode,
                                                                                isScrollControlled: true,
                                                                                context: context,
                                                                                shape: RoundedRectangleBorder(
                                                                                  borderRadius: BorderRadius.vertical(top: Radius.circular(25.0)),
                                                                                  ...
                                                                                  return Container(
                                                                                    ...
                                                                                    child: Padding(
                                                                                      ...
                                                                                      child: Column(
                                                                                        mainAxisAlignment: MainAxisAlignment.center,
                                                                                        children: [
                                                                                          Icon(Icons.check, color: zipchatGreenColor400, size: 40),
                                                                                          SizedBox(
                                                                                            height: 30,
                                                                                          ),
                                                                                          Text(
                                                                                            ...
                                                                                            textAlign: TextAlign.center,
                                                                                            style: TextStyle(
                                                                                              color: pickTextColorBasedOnBgColorAdvanced(Thm.isDarktheme(widget.prefs) ? zipchatDIALOGColorDarkMode : zipchatDIALOGColorLightMode),
                                                                                            ),
                                                                                          )
                                                                                        ],
                                                                                      ),
                                                ...
                                                case 'remove_wallpaper':
                                                  ...
                                                  // zipchat.toast('Wallpaper removed.');
                                                  break;
                                                case 'set_wallpaper':
                                                  Navigator.push(
                                                      context,
  ...
  deleteAllChats() async {
    if (messages.length > 0) {
      zipchat.toast(getTranslated(this.context, 'deleting'));
      await FirebaseFirestore.instance
          .collection(DbPaths.collectionmessages)
          .doc(chatId)
          .get()
