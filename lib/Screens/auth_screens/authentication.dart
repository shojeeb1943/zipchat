...
import 'package:zipchat/widgets/Passcode/passcode_screen.dart';
...
class _AuthenticateState extends State<Authenticate> {
      ...
      child = Material(
          ...
          child: Center(
            ...
            child: Text(
              ...
              style: TextStyle(color: zipchatWhite, fontSize: 18, height: 1.5),
            ),
          )));
    else {
      child = Container();
    }
    return zipchat.getNTPWrappedWidget(child);
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    if (enteredPasscode.length == 4) {
      bool isValid =
          zipchat.getHashedAnswer(enteredPasscode) == widget.passcode;
      ...
      if (isValid) {
        ...
        if (passcodeTries > 3) {
          zipchat.toast('Try after ${math.pow(2, passcodeTries - 3)} minutes');
          zipchat.toast(getTranslated(this.context, 'authfailed'));
          widget.state.pop();
        }
      }
    }
